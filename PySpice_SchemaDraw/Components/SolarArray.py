...
class SolarArray:
    ...
    def draw(self, drawing: schemdraw.Drawing, terminateDist: float = 2, isRight: bool = True):
        ...
        for i in range(self.parallel):        
            ...
            upper = elm.Line().right().at(fst.start).length(2) if isRight else elm.Line().left().at(fst.start).length(2)
            lower = elm.Line().right().at(lst.end).length(2) if isRight else elm.Line().left().at(lst.end).length(2)
            drawing.add(upper)
            drawing.add(lower)
